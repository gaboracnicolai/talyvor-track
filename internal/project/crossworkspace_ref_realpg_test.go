...
// TestProject_CrossWorkspaceTeamIsRefused_RealPG — a project may not belong to another workspace's team.
func TestProject_CrossWorkspaceTeamIsRefused_RealPG(t *testing.T) {
	d := testutil.New(t)
	ctx := context.Background()
	mine, theirs := d.Workspace(t), d.Workspace(t)
	localTeam, foreignTeam := d.Team(t, mine.ID), d.Team(t, theirs.ID)
	s := project.NewStore(d.Pool)

	if got, err := s.Create(ctx, model.Project{
		WorkspaceID: mine.ID, TeamID: foreignTeam.ID, Name: "cross-tenant", Identifier: "XT",
	}); err == nil {
		t.Errorf("Create ACCEPTED a team from another workspace (%s) — the ref guard is not "+
			"refusing. Row created as %v.", foreignTeam.ID, got.ID)
	}

	// MUST STAY GREEN.
	if _, err := s.Create(ctx, model.Project{
		WorkspaceID: mine.ID, TeamID: localTeam.ID, Name: "same-workspace", Identifier: "SW",
	}); err != nil {
		t.Fatalf("Create REFUSED the workspace's own team (%s): %v", localTeam.ID, err)
	}
}
