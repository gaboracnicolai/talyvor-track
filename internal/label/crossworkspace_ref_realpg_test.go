...
// TestLabel_CrossWorkspaceTeamIsRefused_RealPG — a label may not be scoped to another workspace's team.
func TestLabel_CrossWorkspaceTeamIsRefused_RealPG(t *testing.T) {
	d := testutil.New(t)
	ctx := context.Background()
	mine, theirs := d.Workspace(t), d.Workspace(t)
	localTeam, foreignTeam := d.Team(t, mine.ID), d.Team(t, theirs.ID)
	s := label.NewStore(d.Pool)

	if got, err := s.Create(ctx, label.Label{
		WorkspaceID: mine.ID, TeamID: &foreignTeam.ID, Name: "cross-tenant",
	}); err == nil {
	...
	// MUST STAY GREEN.
	if _, err := s.Create(ctx, label.Label{
		WorkspaceID: mine.ID, TeamID: &localTeam.ID, Name: "same-workspace",
	}); err != nil {
